...
ConstantVisitor::ConstantVisitor(const u2 &count, const ConstantInfo *pool) : constant_pool_count(count), constant_pool(pool) {
}
...
std::string ConstantVisitor::visit(const u2 &constantId,
	...
	if (expected && type != *expected)
		throw ClassFormatException(
		    "Constant is not expected:" + std::to_string(type) + " Expected is:" + std::to_string(*expected));
	const u1 *payload = info.info;
	switch (type) {
	case Class: {
		const ConstantClass *classInfo = (const ConstantClass *)payload;
		return "#" + std::to_string(classInfo->name_index) + "\t//" + visit(classInfo->name_index, &utf8);
	}
	case Fieldref:
		break;
	case Methodref: {
		...
		return methodRef->toString() + "\t// " + visit(methodRef->class_index, &c1) + "." + visit(methodRef->name_and_type_index, &c2);
	}
	case InterfaceMethodref:
		// TODO:
		break;
	case String: {
		...
		return "#" + std::to_string(string->string_index) + "\t//" + visit(string->string_index, &utf8);
	}
	case Integer: {
	...
	case NameAndType: {
		...
		return visit(nameAndType->name_index, &utf8) + ":" + visit(nameAndType->descriptor_index, &utf8);
	}
	case Utf8: {
